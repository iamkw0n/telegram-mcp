...
`.env` 에 `TG_API_ID`, `TG_API_HASH` 를 채워 넣고, `MCP_AUTH_TOKEN` 은 랜덤 문자열로 생성해 넣습니다:
...
기본적으로 `http://0.0.0.0:8811/mcp` 에서 Streamable HTTP MCP 엔드포인트가 열립니다. 모든 요청은 `Authorization: Bearer <MCP_AUTH_TOKEN>` 헤더가 있어야 통과됩니다.
...
4. ChatGPT/Claude 커넥터 URL을 그 영구 도메인으로 교체.

## 6. ChatGPT / Claude에 커넥터로 등록

- **URL**: `https://<터널 또는 도메인>/mcp`
- **인증**: Bearer Token / API Key 방식으로 `.env`의 `MCP_AUTH_TOKEN` 값을 입력.

각 서비스의 "커스텀 커넥터/MCP 서버 추가" 설정 화면에서 위 URL과 토큰을 입력하면 됩니다. (UI 경로는 두 서비스 모두 자주 바뀌므로, 설정 메뉴에서 "Connectors" 또는 "MCP" 항목을 찾으세요.)

## 보안 주의사항

- `TG_SESSION_STRING`, `MCP_AUTH_TOKEN`, `TG_API_HASH` 는 모두 비밀값입니다. `.env` 밖으로 노출/커밋하지 마세요.
- `MCP_AUTH_TOKEN` 인증은 정적 토큰 검증(개인 단일 사용자용 간이 방식)입니다. 프로덕션급 다중 사용자 OAuth가 필요하면 별도 확장이 필요합니다.
